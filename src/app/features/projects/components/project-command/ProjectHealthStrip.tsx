...
export function ProjectHealthStrip({ metrics }: { metrics: ProjectCommandMetrics }) {
  ...
  return <div className="space-y-3">
    {metrics.completionRecommended && <div className={`flex items-start gap-2 rounded-xl border px-3.5 py-3 text-[10.5px] ${recommendationTone}`}>
      ...
      <div><span className="font-semibold">Project completion is ready for review.</span> All linked tasks are approved, but the project is still active. Review its activities and mark it completed when appropriate; eFlow has not changed its status automatically.</div>
    </div>}
