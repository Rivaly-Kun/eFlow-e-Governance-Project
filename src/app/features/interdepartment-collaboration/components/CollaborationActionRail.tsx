...
export function CollaborationActionRail({
  ...
  return <div className="space-y-3">
    {isOwner && !departmentOnly && status === "draft" && <button type="button" disabled={busy} onClick={onRequestReview} className="flex w-full items-center justify-center gap-2 rounded-xl bg-violet-700 px-4 py-3 text-[11px] font-['Lexend:Medium',_sans-serif] text-white disabled:opacity-50"><Send size={13} /> Request collaboration review</button>}
    {isOwner && (departmentOnly || readiness?.ready) && status !== "committed" && status !== "archived" && <button type="button" disabled={busy || !hasRevision || !readiness?.ready} onClick={onCommit} className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-[11px] font-['Lexend:Medium',_sans-serif] text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-50"><CheckCircle2 size={13} /> {departmentOnly ? "Publish department proposal" : "Publish proposal"}</button>}
