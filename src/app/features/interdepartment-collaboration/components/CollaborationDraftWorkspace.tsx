...
import { commitCollaborationDraft } from "../services/collaborationCommitService";
  ...
  return <div className="min-h-full bg-neutral-50 p-6 sm:p-8">
    ...
    <div className={`mt-4 grid gap-4 ${committed ? "grid-cols-1" : "xl:grid-cols-[minmax(0,1fr)_300px]"}`}>
      <main>
        ...
        {tab === "budget" && <CollaborationBudgetPanel snapshot={snapshot} editable={isOwner && draft.sourceType === "manual" && state.participants.every((participant) => participant.participationRole === "owner") && !["committed", "archived", "deleted"].includes(draft.status)} onSave={saveRevision} />}
      ...
      {!committed && <aside className="space-y-3">{tab !== "approvals" && <CollaborationReadiness participants={state.participants} approvals={state.approvals} currentRevisionId={draft.currentRevisionId} readiness={state.readiness} organizations={organizations} departmentOnly={departmentOnly} />}
        <CollaborationActionRail departmentOnly={departmentOnly} isOwner={isOwner} ownerName={ownerOrg?.name} status={draft.status} readiness={state.readiness} busy={busy} hasRevision={Boolean(draft.currentRevisionId)} onRequestReview={() => act(async () => { await requestCollaborationReview(draftId); }, "Collaboration review requested.")} onCommit={() => act(async () => { await commitCollaborationDraft(draftId, draft.currentRevisionId!); onCommitted(); }, departmentOnly ? "Department proposal published. Operational projects and tasks are now available." : "Proposal published. Operational projects and tasks are now available.")} onDelete={(reason) => act(async () => { await deleteCollaborationDraft(draftId, reason); onBack(); }, departmentOnly ? "Department proposal draft deleted." : "Collaboration draft deleted with its governance history retained.")} />
      </aside>}
    </div>
  </div>;
}
