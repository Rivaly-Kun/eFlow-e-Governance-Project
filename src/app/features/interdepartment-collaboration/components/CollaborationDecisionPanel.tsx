  ...
  return (
    <section className="rounded-2xl border border-neutral-200 bg-white p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button onClick={() => setDecision("approved")} className="rounded-lg bg-emerald-600 px-3 py-2 text-[10px] font-['Lexend:Medium',_sans-serif] text-white">Approve current revision</button>
      ...
      {decision && <div className="mt-3 rounded-xl bg-neutral-50 p-3"><textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={3} placeholder={decision === "approved" ? "Optional endorsement note" : "Reason required"} className="w-full resize-none rounded-lg border border-neutral-200 bg-white px-3 py-2 text-[10px] outline-none" /><div className="mt-2 flex justify-end gap-2"><button onClick={() => { setDecision(null); setReason(""); }} className="px-3 py-2 text-[10px] text-neutral-500">Cancel</button><button disabled={busy || !selected || (decision !== "approved" && !reason.trim())} onClick={async () => { await onDecide(decision, reason); setDecision(null); setReason(""); }} className="rounded-lg bg-neutral-900 px-3 py-2 text-[10px] text-white disabled:opacity-40">Confirm decision</button></div></div>}
    </section>
  );
}
